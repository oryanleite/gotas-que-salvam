...
function Hero({onSelect,onGps,status,setStatus}:{onSelect:(r:PositionReference)=>void;onGps:()=>void;status:LocationStatus;setStatus:(s:LocationStatus)=>void}){return <section id="inicio" className="hero expanded-hero"><div className="shell hero-grid"><div className="hero-copy"><span className="eyebrow"><HeartHandshake/>Tecnologia a serviço da solidariedade</span><h1>Encontre onde <em>doar sangue</em> perto de você</h1><p>Encontre hemocentros, bancos de sangue e hospitais que recebem doações na Grande São Paulo.</p><LocationSearch onSelect={onSelect} onGps={onGps} status={status} setStatus={setStatus}/><div className="trust-row"><span><ShieldCheck/>Sem cadastro</span><span><ShieldCheck/>Sem histórico</span><span><ShieldCheck/>Fontes oficiais</span></div></div><div className="hero-visual" aria-hidden="true"><div className="legal-lines"><span/><span/><span/></div><div className="drop-large"><div className="balance"><span/><b/><span/></div><HeartHandshake/></div><div className="floating-card card-a"><MapPin/><span><strong>{PUBLIC_DONATION_CENTERS.length} locais verificados</strong>Na Grande São Paulo</span></div><div className="floating-card card-b"><ShieldCheck/><span><strong>Privacidade</strong>Busca não armazenada</span></div><span className="pulse-dot dot-a"/><span className="pulse-dot dot-b"/></div></div><div className="shell"><HeroDonationCounter/></div></section>}
