...
import { ArrowRight, CalendarCheck, Check, CheckCircle2, CircleAlert, Clock3, ExternalLink, AtSign, Flag, Globe, HeartHandshake, Info, Landmark, LocateFixed, Loader2, Map as MapIcon, MapPin, Menu, Navigation, Phone, Scale, Search, Share2, ShieldCheck, Sparkles, Stethoscope, X } from "lucide-react";
...
const INSTAGRAM_HANDLE = "asgotasquesalvam";
const INSTAGRAM_URL = `https://www.instagram.com/${INSTAGRAM_HANDLE}`;
// Tempo de espera antes de consultar o OpenStreetMap, para não disparar uma
// requisição a cada tecla digitada (política de uso do Nominatim).
const REMOTE_SEARCH_DEBOUNCE_MS = 450;
...
function LocationSearch({onSelect,onGps,status,setStatus}: {onSelect:(r:PositionReference)=>void;onGps:()=>void;status:LocationStatus;setStatus:(s:LocationStatus)=>void}){
  ...
  // Um único estado para a busca remota, sempre carimbado com o termo que o
  // originou. Assim o efeito nunca precisa "limpar" nada de forma síncrona:
  // basta o termo não bater para o resultado antigo ser ignorado.
  const [remoteState,setRemoteState]=useState<{query:string;items:GeocodeResult[];failed:boolean}>({query:"",items:[],failed:false});
  // Rótulo já escolhido: evita reabrir a busca remota logo depois de o usuário
  // selecionar uma sugestão (o texto do campo passa a ser o próprio rótulo).
  const [chosen,setChosen] = useState("");

  const suggestions=useMemo(()=>findLocationSuggestions(LOCATION_REFERENCES,query,7),[query]);
  const isCep = normalizeCep(query)!==null;
  const shouldSearchRemote = query.trim().length>=3 && suggestions.length===0 && !isCep && query!==chosen;

  const settled = shouldSearchRemote && remoteState.query===query;
  const remote = settled ? remoteState.items : [];
  const remoteLoading = shouldSearchRemote && !settled;
  const remoteFailed = settled && remoteState.failed;

  // Busca no OpenStreetMap apenas quando a lista local não tem nada a oferecer.
  useEffect(()=>{
    ...
    const timer = setTimeout(async ()=>{
      try{
        ...
        if(!cancelled) setRemoteState({query,items:found,failed:false});
      }catch(err){
        if(cancelled || (err instanceof DOMException && err.name==="AbortError")) return;
        setRemoteState({query,items:[],failed:true});
      }
    }, REMOTE_SEARCH_DEBOUNCE_MS);
    return ()=>{ cancelled=true; controller.abort(); clearTimeout(timer); };
  },[query,shouldSearchRemote]);

  const choose=(location:LocationReference,source:"manual"|"cep"="manual")=>{
    setChosen(location.label);
    setQuery(location.label); setError(""); setRemoteState({query:"",items:[],failed:false});
    onSelect({latitude:location.latitude,longitude:location.longitude,label:location.label,neighborhood:location.kind==="bairro"?location.neighborhood:undefined,source});
  };
  ...
  const searchCep=async()=>{
    ...
    try{
      ...
      const data=await response.json() as {erro?:boolean;bairro?:string;localidade?:string};
      if(data.erro||!data.localidade) throw new Error();
      const exact=LOCATION_REFERENCES.find((l)=>normalizeText(l.neighborhood)===normalizeText(data.bairro??"")&&normalizeText(l.city)===normalizeText(data.localidade??""));
      const city=LOCATION_REFERENCES.find((l)=>l.kind==="municipio"&&normalizeText(l.city)===normalizeText(data.localidade??""));
      const ref=exact??city;
      if(!ref){ setError("CEP localizado, mas ainda não temos uma referência geográfica segura para essa região."); return true; }
      choose({...ref,label:data.bairro?`${data.bairro} — ${data.localidade}`:data.localidade,kind:"cep"},"cep");
    }catch{
      setError("Não foi possível consultar esse CEP. Verifique os oito números e tente novamente.");
    }finally{ setLoading(false); }
    return true;
  };

  const search=async()=>{
    if(await searchCep()) return;
    if(suggestions[0]){ choose(suggestions[0]); return; }
    if(remote[0]){ choose(remote[0]); return; }
    if(query.trim().length<3){ setError("Digite pelo menos três letras do bairro, da cidade ou o CEP."); return; }
    // Nada em cache ainda: consulta o OpenStreetMap na hora.
    setLoading(true); setError("");
    try{
      const found=await searchPlaces(query);
      if(found[0]){ setRemoteState({query,items:found,failed:false}); choose(found[0]); return; }
      setError("Não encontramos esse local na Grande São Paulo. Tente outro bairro, o município ou o CEP.");
    }catch{
      setError("Não foi possível consultar a busca de endereços agora. Tente novamente ou use seu CEP.");
    }finally{ setLoading(false); }
  };

  const showList = query.length>=2;
  const nothingAtAll = !isCep && suggestions.length===0 && remote.length===0 && !remoteLoading;

  return <div id="busca" className="hero-search-card">
    <label>Onde você está?</label>
    <Command shouldFilter={false} className="location-command">
      <CommandInput value={query} onValueChange={(v)=>{setQuery(v);setError("");setStatus("idle")}} placeholder="Digite seu bairro, cidade ou CEP" onKeyDown={(e)=>{if(e.key==="Enter"){e.preventDefault();void search()}}}/>
      {showList&&<CommandList>
        <CommandEmpty>{isCep?"Pressione Enter para consultar o CEP":nothingAtAll?(remoteFailed?"Busca de endereços indisponível agora. Tente o CEP.":"Nenhum local encontrado."):""}</CommandEmpty>
        {suggestions.length>0&&<CommandGroup heading="Sugestões de localização">
          {suggestions.map((item: LocationReference)=><CommandItem key={item.id} value={item.label} onSelect={()=>choose(item)}><MapPin/>{item.label}</CommandItem>)}
        </CommandGroup>}
        {remoteLoading&&<CommandGroup heading="Buscando no OpenStreetMap"><CommandItem value="__loading" disabled><Loader2 className="spin"/>Procurando esse endereço...</CommandItem></CommandGroup>}
        {remote.length>0&&<CommandGroup heading="Resultados do OpenStreetMap">
          {remote.map((item)=><CommandItem key={item.id} value={item.label} onSelect={()=>choose(item)}><Globe/>{item.label}</CommandItem>)}
        </CommandGroup>}
      </CommandList>}
    </Command>
    <button className="search-submit" onClick={()=>void search()} disabled={loading}>{loading?"Consultando...":"Pesquisar"}<Search size={17}/></button>
    <div className="search-or"><span/>ou<span/></div>
    <Button onClick={onGps} disabled={status==="requesting"} className="gps-button"><LocateFixed/>{status==="requesting"?"Buscando sua localização...":"Usar minha localização"}</Button>
    <p className="search-privacy"><ShieldCheck/>Sua localização é utilizada somente para encontrar locais próximos e não é armazenada.</p>{(remoteLoading||remote.length>0)&&<p className="search-privacy subtle"><Globe/>Este local não está na nossa lista: consultamos o OpenStreetMap para encontrá-lo. <a href="/privacidade">Saiba mais</a>.</p>}
    {error&&<p className="search-error" role="alert"><CircleAlert/>{error}</p>}
    {(status==="denied"||status==="unavailable")&&<p className="search-error" role="status"><CircleAlert/>{status==="denied"?"Acesso à localização negado. Digite seu bairro, cidade ou CEP.":"Localização indisponível. Digite seu bairro, cidade ou CEP."}</p>}
  </div>
}
